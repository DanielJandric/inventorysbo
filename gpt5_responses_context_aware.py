#!/usr/bin/env python3
"""
Solution GPT-5 CORRECTE - Utilise l'API Responses avec gestion automatique du contexte
Basé sur la documentation OpenAI officielle : previous_response_id pour le contexte
"""
...
class GPT5ResponsesContextManager:
    """Gestionnaire GPT-5 utilisant CORRECTEMENT l'API Responses avec contexte automatique"""
    ...
    def __init__(self):
        ...
        self.conversation_history: List[str] = []  # Stockage des IDs de réponses
    ...
    def generate_response_with_context(self, user_input: str, system_prompt: str = "", context: str = "") -> Dict[str, Any]:
        """
        Génère une réponse en utilisant CORRECTEMENT l'API Responses
        Avec gestion automatique du contexte via previous_response_id
        """
        try:
    ...
    def generate_response_with_fallback(self, user_input: str, system_prompt: str = "", context: str = "") -> Dict[str, Any]:
        ...
        Mais essaie d'abord Responses correctement avec contexte
        """
        # Tentative 1: API Responses avec gestion automatique du contexte
        result = self.generate_response_with_context(user_input, system_prompt, context)
        if result["success"]:
            return result
    ...
    def health_check(self) -> Dict[str, Any]:
        ...
        # Test API Responses
        try:
            result = self.generate_response_with_context(test_input, "Tu es un assistant.")
...
def get_gpt5_response_with_context(user_input: str, system_prompt: str = "", context: str = "", use_fallback: bool = True) -> Dict[str, Any]:
    """
    Fonction utilitaire pour obtenir une réponse GPT-5 via l'API Responses
    Avec gestion automatique du contexte via previous_response_id
    """
    manager = GPT5ResponsesContextManager()
    
    if use_fallback:
        return manager.generate_response_with_fallback(user_input, system_prompt, context)
    else:
        return manager.generate_response_with_context(user_input, system_prompt, context)
...
if __name__ == "__main__":
    ...
    # Test de génération via API Responses avec contexte
    print("\n🤖 TEST API RESPONSES AVEC CONTEXTE AUTOMATIQUE")
    result = manager.generate_response_with_context(
    ...
    follow_up = manager.generate_response_with_context(
